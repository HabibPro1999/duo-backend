...
import { firebaseAuthMock, createMockDecodedToken } from '../../../tests/mocks/firebase.js';
import {
  createMockUser,
...
describe('requireAuth', () => {
  ...
  describe('Authorization Header Validation', () => {
    it('should throw 401 when authorization header is missing', async () => {
      ...
      try {
        ...
        expect((error as AppError).message).toBe('Missing or invalid authorization header');
      }
    });
    ...
    it('should throw 401 when Bearer is present but token is missing', async () => {
      const request = createMockRequest({ headers: { authorization: 'Bearer ' } });
  ...
  describe('Successful Authentication', () => {
    ...
    it('should work with client admin users', async () => {
      ...
      const clientAdmin = createMockClientAdmin(clientId, { id: 'client-admin-123' });
...
describe('requireRole', () => {
  ...
  describe('Role Validation', () => {
    it('should throw 403 when user role is not in allowed roles', async () => {
      ...
      const request = createMockRequest({ user: clientAdmin } as Partial<FastifyRequest>);
      const middleware = requireRole(UserRole.SUPER_ADMIN);
    ...
    it('should pass when user role matches single allowed role', async () => {
      ...
      const request = createMockRequest({ user: superAdmin } as Partial<FastifyRequest>);
      const middleware = requireRole(UserRole.SUPER_ADMIN);
    ...
    it('should pass when user role matches one of multiple allowed roles', async () => {
      const clientAdmin = createMockClientAdmin('client-123');
      const request = createMockRequest({ user: clientAdmin } as Partial<FastifyRequest>);
      const middleware = requireRole(UserRole.SUPER_ADMIN, UserRole.CLIENT_ADMIN);
...
describe('requireSuperAdmin', () => {
  ...
  it('should throw 401 when user is not authenticated', async () => {
    ...
    await expect(requireSuperAdmin(request, mockReply)).rejects.toThrow(AppError);
  ...
  it('should pass when user is super admin', async () => {
    ...
    await expect(requireSuperAdmin(request, mockReply)).resolves.toBeUndefined();
  });
});
...
describe('requireAdmin', () => {
  ...
  it('should throw 403 for unknown role values', async () => {
    ...
    const request = createMockRequest({ user: unknownRoleUser } as Partial<FastifyRequest>);
...
describe('canAccessClient', () => {
  ...
  describe('Client Admin Access', () => {
    it('should allow client admin to access their own client', () => {
      const clientAdmin = { role: UserRole.CLIENT_ADMIN, clientId: 'client-123' };
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle UUID format client IDs correctly', () => {
      ...
      expect(canAccessClient(clientAdmin, 'a1b2c3d4-e5f6-7890-abcd-ef1234567891')).toBe(false);
    });
  });
});
...
describe('Auth Middleware Integration', () => {
  ...
  it('should fail requireSuperAdmin after successful auth for client admin', async () => {
    ...
    const decodedToken = createMockDecodedToken({ uid: 'client-admin-123' });
    const clientAdmin = createMockClientAdmin('client-456', { id: 'client-admin-123' });
