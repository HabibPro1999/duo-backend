...
describe('Email Variable Service', () => {
  describe('BASE_VARIABLES', () => {
    it('should contain registration variables', () => {
      const registrationVars = BASE_VARIABLES.filter((v) => v.category === 'registration');
    ...
    it('should contain payment variables', () => {
      const paymentVars = BASE_VARIABLES.filter((v) => v.category === 'payment');
  ...
  describe('getAvailableVariables', () => {
          ...
          steps: [
            {
              id: 'step-1',
              title: 'Details',
              fields: [{ id: 'custom_field', type: 'text', label: 'Custom Field' }],
            },
          ],
        },
      });
    ...
    it('should provide appropriate examples for different field types', async () => {
      ...
      expect(result.find((v) => v.id === 'form_user_email')?.example).toBe('user@example.com');
      expect(result.find((v) => v.id === 'form_user_phone')?.example).toBe('+216 12 345 678');
      expect(result.find((v) => v.id === 'form_age')?.example).toBe('42');
      expect(result.find((v) => v.id === 'form_birth_date')?.example).toBe('March 15, 2025');
      expect(result.find((v) => v.id === 'form_notes')?.example).toBe('Long text content...');
    });
  });
  ...
  describe('buildEmailContextWithAccess', () => {
    ...
    it('should resolve access type IDs to names', async () => {
      ...
      const mockAccessTypes = [
        createMockEventAccess({ id: 'access-1', name: 'Workshop A', type: 'WORKSHOP' }),
        createMockEventAccess({ id: 'access-2', name: 'Gala Dinner', type: 'DINNER' }),
        createMockEventAccess({ id: 'access-3', name: 'Workshop B', type: 'WORKSHOP' }),
      ];
      ...
      expect(context.selectedAccess).toBe('Workshop A, Gala Dinner, Workshop B');
  ...
  describe('resolveVariables', () => {
    ...
    it('should sanitize HTML in variable values', () => {
      ...
      const template = 'Hello {{firstName}}';
      const result = resolveVariables(template, contextWithHtml);
  ...
  describe('sanitizeForHtml', () => {
    ...
    it('should handle complex HTML injection attempts', () => {
      const malicious = '<script>document.cookie</script><img onerror="alert(1)" src="x">';
