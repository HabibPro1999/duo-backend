...
describe('Sponsorship Utils', () => {
  ...
  describe('determineSponsorshipStatus', () => {
    ...
    it('should keep CANCELLED status regardless of usage count', () => {
      ...
      const statusWithUsages = determineSponsorshipStatus({ status: 'CANCELLED' }, 5);
...
describe('Sponsorships Service', () => {
  ...
  describe('createSponsorshipBatch', () => {
    ...
    it('should create batch with sponsorships successfully', async () => {
      ...
      const mockBatch = createMockSponsorshipBatch({ id: batchId, eventId, formId });
    ...
    it('should create multiple sponsorships for multiple beneficiaries', async () => {
        ...
        const txMock = {
          ...
          sponsorship: {
            create: vi.fn().mockImplementation(() => {
              sponsorshipCount++;
              return sponsorshipCount === 1 ? mockSponsorship1 : mockSponsorship2;
            }),
      ...
      const result = await createSponsorshipBatch(eventId, formId, inputWithMultipleBeneficiaries);
        ...
        expect.objectContaining({
          where: expect.objectContaining({
            OR: expect.arrayContaining([
              ...
              expect.objectContaining({ beneficiaryEmail: expect.any(Object) }),
            ]),
          }),
        })
      );
  ...
  describe('getSponsorshipById', () => {
    it('should return sponsorship with full details', async () => {
      ...
      const mockSponsorship = {
        ...createMockSponsorship({ id: sponsorshipId, eventId, coveredAccessIds: [accessId] }),
        batch: createMockSponsorshipBatch({ eventId }),
        usages: [],
      };
      const mockAccess = createMockEventAccess({ id: accessId, name: 'Workshop A', price: 50 });
  ...
  describe('updateSponsorship', () => {
    ...
    it('should recalculate total when coverage changes', async () => {
      ...
      const accessId = faker.string.uuid();
      const mockSponsorship = {
        ...createMockSponsorship({ id: sponsorshipId, eventId, coversBasePrice: true }),
        usages: [],
      };
    ...
    it('should delegate to cancelSponsorship when status is CANCELLED', async () => {
      ...
      const result = await updateSponsorship(sponsorshipId, { status: 'CANCELLED' });
  ...
  describe('cancelSponsorship', () => {
    it('should cancel sponsorship without usages', async () => {
      ...
      prismaMock.$transaction.mockImplementation(async (fn: TxCallback) => {
        const txMock = {
          sponsorship: {
            update: vi.fn().mockResolvedValue({ ...mockSponsorship, status: 'CANCELLED' }),
          },
        };
        return fn(txMock);
      });
    ...
    it('should unlink from registrations when cancelling', async () => {
      ...
      prismaMock.$transaction.mockImplementation(async (fn: TxCallback) => fn(mockTxFns));
      prismaMock.eventAccess.findMany.mockResolvedValue([]);
  ...
  describe('linkSponsorshipToRegistration', () => {
    ...
    it('should link sponsorship successfully', async () => {
      ...
      prismaMock.sponsorship.findUnique.mockResolvedValue(mockSponsorship);
      prismaMock.registration.findUnique.mockResolvedValue(asMock(mockRegistration));
    ...
    it('should throw when sponsorship not found', async () => {
      ...
      await expect(
        linkSponsorshipToRegistration(sponsorshipId, registrationId, adminUserId)
      ).rejects.toMatchObject({
        statusCode: 404,
        code: ErrorCodes.NOT_FOUND,
      });
    ...
    it('should throw when sponsorship is cancelled', async () => {
      const mockSponsorship = {
        ...createMockSponsorship({ id: sponsorshipId, eventId, status: 'CANCELLED' }),
        usages: [],
      };
    ...
    it('should throw when registration not found', async () => {
      const mockSponsorship = {
        ...createMockSponsorship({ id: sponsorshipId, eventId, status: 'PENDING' }),
        usages: [],
      };
    ...
    it('should throw when sponsorship is already linked to this registration', async () => {
      ...
      prismaMock.sponsorship.findUnique.mockResolvedValue(asMock(mockSponsorship));
      prismaMock.registration.findUnique.mockResolvedValue(asMock(mockRegistration));
      prismaMock.sponsorshipUsage.findUnique.mockResolvedValue(asMock({
        id: faker.string.uuid(),
        sponsorshipId,
        registrationId,
      }));
  ...
  describe('linkSponsorshipByCode', () => {
    ...
    it('should find sponsorship by code and link it', async () => {
      ...
      prismaMock.sponsorship.findFirst.mockResolvedValue(asMock(mockSponsorship));
      prismaMock.sponsorship.findUnique.mockResolvedValue(asMock(mockSponsorship));
      prismaMock.sponsorshipUsage.findUnique.mockResolvedValue(null);
      ...
      const result = await linkSponsorshipByCode(registrationId, code, adminUserId);
  ...
  describe('unlinkSponsorshipFromRegistration', () => {
    ...
    it('should unlink sponsorship successfully', async () => {
      ...
      prismaMock.sponsorship.findUnique.mockResolvedValue(asMock({ status: 'USED' }));
  ...
  describe('getSponsorshipClientId', () => {
    it('should return client ID for sponsorship', async () => {
      ...
      prismaMock.sponsorship.findUnique.mockResolvedValue(asMock({
        id: sponsorshipId,
        event: { clientId },
      }));
  ...
  describe('getLinkedSponsorships', () => {
    ...
    it('should return linked sponsorships with usage info', async () => {
      ...
      prismaMock.sponsorshipUsage.findMany.mockResolvedValue(asMock(mockUsages));
