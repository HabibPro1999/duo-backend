...
import { createMockForm, createMockEvent, createMockClient } from '../../../tests/helpers/factories.js';
import {
  createForm,
  getFormById,
  getFormByEventSlug,
...
describe('Forms Service', () => {
  ...
  describe('getFormById', () => {
    it('should return form when found', async () => {
      ...
      expect(result).toEqual(mockForm);
      expect(prismaMock.form.findUnique).toHaveBeenCalledWith({ where: { id: formId } });
    });
  ...
  describe('getFormByEventSlug', () => {
    it('should return form with relations for OPEN event', async () => {
      ...
      prismaMock.form.findFirst.mockResolvedValue(formWithRelations as Form & { event: unknown });
  ...
  describe('updateForm', () => {
    ...
    it('should increment schemaVersion when schema changes', async () => {
      ...
      const newSchema = { steps: [{ id: 'new-step', title: 'New', fields: [] }] };
    ...
    it('should throw when form not found', async () => {
      ...
      await expect(updateForm('non-existent', { name: 'Test' })).rejects.toThrow(AppError);
      await expect(updateForm('non-existent', { name: 'Test' })).rejects.toMatchObject({
        statusCode: 404,
        code: ErrorCodes.NOT_FOUND,
      });
    });
  ...
  describe('deleteForm', () => {
    it('should delete form when found', async () => {
      ...
      expect(prismaMock.form.delete).toHaveBeenCalledWith({ where: { id: formId } });
    });
  ...
  describe('formExists', () => {
    it('should return true when form exists', async () => {
      ...
      expect(prismaMock.form.count).toHaveBeenCalledWith({ where: { id: formId } });
    });
