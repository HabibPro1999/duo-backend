  ...
  const form = await prisma.form.findFirst({
    where: {
      ...
      event: {
        ...
        client: { active: true, enabledModules: { has: "registrations" } },
      },
      active: true,
    },
    include: {
...
export async function updateSponsorshipSettings(
  ...
  if (settings.sponsorshipMode !== currentMode) {
    return withTxnRetry(
      () =>
        prisma.$transaction(
          async (tx) => {
            ...
            const schema =
              currentForm.schema as unknown as SponsorFormSchemaJson;
      ...
      { label: "updateSponsorshipSettings" },
    );
  }
  ...
  const form = await prisma.form.findFirst({
    where: {
      ...
      event: {
        ...
        status: "OPEN",
        endDate: { gte: new Date() },
        client: { active: true, enabledModules: { has: "sponsorships" } },
      },
      active: true,
    },
    include: {
